...
int main(void)
{
	...
	while (waitPeriod++ < 20) // wait time for bootloader until launching interpreter (20 * 100mS) = 2sec
	{
		...
		if (rxChar != '\0')
		{ // if received something either go into program load, or branch to REPL
			charRx = true;
			PORTC = 0x20;
			...
			send_byte('\n');
			if (rxChar == ':')
			{ // if a ':' was first char, then branch to REPL
				Interpreter i;
#ifdef LCD_SUPPORT
				InitLCD(false);
#endif
	...
	{ // if we got data...
		while (rxChar != '\0')
		{ // do receive-and-store until we get a NULL byte
